...
//       console.log(
//         "🔔 New message received via socket for contactId:",
//         response
//       );

//       // Start calling handleRefresh every 2 seconds (for example)
...
//         console.log("🛑 Stopped refreshing after 10 seconds");
//       }, 10000);
//     };
...
export function ConversationsPage({ locationId }: ConversationsPageProps = {}) {
  ...
  // --- Refresh loop control: prevent overlapping refreshes and support cleanup ---
  const refreshRunningRef = useRef(false);
  const refreshIntervalRef = useRef<number | null>(null);
  const refreshTimeoutRef = useRef<number | null>(null);

  const startRefreshLoop = useCallback(
    (intervalMs = 1000, durationMs = 10000) => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (response: any) => {
      try {
        ...
        // Immediately update UI from socket (best effort)
        if (response) {
          updateConversationFromSocket(response);
        }

        // Start periodic refresh for 10 seconds (1s interval)
        startRefreshLoop(1000, 10000);
      } catch (err) {
    ...
    return () => {
      socket.off("new_message", handleNewMessage);
      // clear any loops when unsubscribing
      if (refreshIntervalRef.current) {
        clearInterval(refreshIntervalRef.current);
        refreshIntervalRef.current = null;
      }
  ...
  }, [socket, updateConversationFromSocket, startRefreshLoop]);
